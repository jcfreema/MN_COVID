...
def get_testing_data(County):
    apiKey = 'ed176acd0337434fbca50466a20af67b' #get your own API key from covidactnow.org
    if len(County) == 1:
        #print(len(County))
        ...
        st.text('Getting testing data for Minnesota State')
        path1 = 'https://data.covidactnow.org/latest/us/states/MN.OBSERVED_INTERVENTION.timeseries.json'
        df = json.loads(requests.get(path1).text)
        data = pd.DataFrame.from_dict(df['actualsTimeseries'])
        data['Date'] = pd.to_datetime(data['date'])
        data = data.set_index('Date')
...
def plot_state():
    ...
    def get_testing_data_state():
            ...
            county_confirmed = confirmed[confirmed.Province_State == 'Minnesota']
            ...
            county_deaths = deaths[deaths.Province_State == 'Minnesota']
            population = county_deaths.Population.values.sum()
    ...
    st.subheader('Current situation of COVID-19 cases in Minnesota ('+ str(today)+')')
    c1 = st.container()
    c2 = st.container()
    c3 = st.container()
    ...
    with c2:
        ...
        with a1:
            #f = FIPSs[FIPSs.County == C].FIPS.values[0]
            components.iframe("https://covidactnow.org/embed/us/minnesota-mn", width=350, height=365, scrolling=False)
...
t1, t2 = st.columns(2)
with t1:
    st.markdown('# Minnesota COVID-19 Data and Reporting')
...
st.write("")
st.markdown("""
COVID-Local provides basic key metrics against which to assess pandemic response and progress. 

*Daily testing data currently available for the state of Minnesota*
...
if sidebar_selection == 'Select Counties':
    ...
    MN_counties = confirmed[confirmed.Province_State == 'Minnesota'].Admin2.unique().tolist()
    counties = st.multiselect('', MN_counties, default=['Hennepin', 'Ramsey', 'Dakota', 'Anoka'])
    # Limit to the first 5 counties.
    counties = counties[:5]
    if not counties:
        # If no counties are specified, just plot the state.
        st.markdown('> No counties were selected, falling back to showing statistics for Minnesota state.')
        plot_state()
    else:
        ...
        for c in counties:
            ...
            with st.expander(f"Expand for {c} County Details"):
                plot_county([c])
elif sidebar_selection == 'Minnesota':
    plot_state()
